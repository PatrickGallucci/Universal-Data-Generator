...
A configuration sample for every target is in [samples/targets/](../../samples/targets/).

## Target list

All catalog storage targets are implemented. New and Update apply by key where the target supports it;
Delete removes by key or, for streaming targets, emits a tombstone event.
...
Every record carries a change envelope so a consumer can apply New, Update, and Delete as change events.
File and lake targets write these fields into each record; streaming targets carry them in the event body
...
See [API: Targets](../api/targets.md) for the registration types and
[ADR-0007](../architecture/decisions/adr-0007-di-sink-registration.md) for the decision behind the model.
