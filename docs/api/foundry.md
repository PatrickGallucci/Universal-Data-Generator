...
`GenerateAsync` builds a structured-output request from the entity's semantic attributes, calls the
Foundry chat deployment, and coerces each returned value to its CDM type. When no API key is supplied,
it authenticates with `DefaultAzureCredential`.
